...
def increref(i):
  ...
  if reference[i] == 5:
    ...
    if i != 0:
      increref(i - 1)
  #if i != 0:
  #  if reference[i - 1] % 2 == reference[i] % 2 and reference[i - 1] != reference[i]:
  #    increref(i)
  

def actionref(i, tailleref, saveGrille1, positionsave): #a modifier si le cas k == 0 existe
  for p in range(tailleref):
    k = reference[p]
    if k <=2:
      if k == 1:
        if not droitesave(p, saveGrille1, positionsave):
          return p
      else:
        if not bassave(p,saveGrille1, positionsave):
          return p
    else:
      if k == 3:
        if not gauchesave(p,saveGrille1, positionsave):
          return p
      else:
        if not hautsave(p,saveGrille1, positionsave):
          return p
  return i
...
def les2(saveGrille1): #verifie si on peut aller au cercle de fin
  for ligne in range(7):
    for colonne in range(7):
    ...
    if saveGrille1[positionsave[0]][depla] != 0 and saveGrille1[positionsave[0]][depla] != 2:
      ...
      if saveGrille1[positionsave[0]][depla] == 2:
        if les2(saveGrille1):
          ...
          return True
  return False #On ne peut pas aller a droite
    ...
    if saveGrille1[depla][positionsave[1]] != 0 and saveGrille1[depla][positionsave[1]] != 2:
      ...
      if saveGrille1[depla][positionsave[1]] == 2:
        if les2(saveGrille1):
          ...
          return True
  return False #On ne peut pas aller en bas
    ...
    if saveGrille1[positionsave[0]][positionsave[1] - depla] != 0 and saveGrille1[positionsave[0]][positionsave[1] - depla] != 2:
      ...
      if saveGrille1[positionsave[0]][positionsave[1] - depla] == 2:
        if les2(saveGrille1):
          ...
          return True
  return False #On ne peut pas aller a gauche
    ...
    if saveGrille1[positionsave[0] - depla][positionsave[1]] != 0 and saveGrille1[positionsave[0] - depla][positionsave[1]] != 2:
      ...
      if saveGrille1[positionsave[0] - depla][positionsave[1]] == 2:
        if les2(saveGrille1):
          ...
          return True
  return False #On ne peut pas aller en haut
...
def solve(grille):
  ...
  i = 0 #indice dans ref 
  #debut de la boucle de solution
  while finish(saveGrille1):
    ...
    increref(i)
    coupre()
    i = actionref(i, tailleref, saveGrille1, positionsave)
    
    fichier.write(str(reference))
    fichier.write("\n")
  #fin de la boucle de solution
  fichier.close()
