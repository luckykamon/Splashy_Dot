...
def increref(o):
  for k in range(o + 1, len(reference) - 1):
    reference[k] = 1
  if o != len(reference) - 1:
    reference[len(reference) - 1] = 0
  i = o
  a = o
  while i == a:
    reference[i] += 1
...
def actionref(tailleref, saveGrille1, positionsave):
  for p in range(tailleref):
    k = reference[p]
    if k == 1:
      droitesave(p, saveGrille1, positionsave)
    else:
      if k == 2:
        bassave(p, saveGrille1, positionsave)
      else:
        if k == 3:
          gauchesave(p, saveGrille1, positionsave)
        else:
          if k == 4:
            hautsave(p, saveGrille1, positionsave)
