...
def increref():
  ...
  #fichier.close()
  #print reference
  #print ((reference[-1] - 1) * 33 + (reference[-2] - 1) * 11 + (reference[-3] - 1) * 3)
  i = 0
  a = 0
  while i == a:
    reference[i] += 1
...
def actionref(saveGrille1, positionsave):
  for k in reference:
    if k != 0:
      if k == 1:
        droitesave(saveGrille1, positionsave)
      else:
        if k == 2:
          bassave(saveGrille1, positionsave)
        else:
          if k == 3:
            gauchesave(saveGrille1, positionsave)
          else:
            if k == 4:
              hautsave(saveGrille1, positionsave)
...
def droitesave(saveGrille1, positionsave):
...
def bassave(saveGrille1, positionsave):
...
def gauchesave(saveGrille1, positionsave):
...
def hautsave(saveGrille1, positionsave):
...
def solve(grille):
  ...
  while finish(saveGrille1):
    ...
    increref()
    if coupre():
      actionref(saveGrille1, positionsave)
